...
GET    /api/cameras/{id}/live          # connection descriptor: { id, display_name, protocol, webrtc_url }
...
Three boundaries:
1. **iOS → gateway:** Sign in with Apple identity token → gateway issues HS256 JWT
...
A new Apple sign-in is admitted if **either** the email is allowlisted **or** a
valid invite code is presented (`invite_code` in the `POST /api/auth/apple` body).
...
- Push, either:
  - **Direct:** `APNS_KEY_PATH` + `APNS_KEY_ID` + `APNS_TEAM_ID` + `APNS_BUNDLE_ID`.
  - **Relay:** `RELAY_URL` + `RELAY_REGISTRATION_SECRET`.
